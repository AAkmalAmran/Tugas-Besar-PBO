...
public class Player {
    ...
    public void insertPlayer(String playerName) {
        try (Connection conn = db.getConnection()) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(insertQuery)) {
                ...
                pstmt.setInt(2, 1); // Default id_weapon
    ...
    public void updatePlayerWeapon(String playerName, int newIdWeapon) {
        String updateQuery = "UPDATE player SET id_weapon = ? WHERE nama_player = ?";
    
        try (Connection conn = db.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(updateQuery)) {
            // Set parameter
            pstmt.setInt(1, newIdWeapon);
            ...
            // Feedback untuk user
            if (rowsAffected > 0) {
                System.out.println("Weapon updated successfully for player: " + playerName);
                // Perbarui instance jika cocok
                if (this.playerName != null && this.playerName.equals(playerName)) {
                    this.idWeapon = newIdWeapon;
                }
            } else {
                System.out.println("Player with name '" + playerName + "' does not exist.");
            }
